...
$stmt = $db->prepare("SELECT id, total_price, payment_method, created from Orders WHERE user_id = :user ORDER BY id desc LIMIT :offset, :count ");
...
$stmt->bindValue(":user", get_user_id(), PDO::PARAM_INT);
...
<?php if($results && count($results) > 0):?>
    <?php foreach($results as $r):?>
        <div class="order">
            <?php echo("Order ID: " . $r['id'] . " | Payed with " . $r['payment_method'] . " Total: $" . $r['total_price']); ?>
        </div>
    <?php endforeach;?>
